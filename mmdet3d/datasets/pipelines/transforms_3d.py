...
class RandomAugImageMultiViewImage(object):
    ...
    def img_transform_core(self, img, resize_dims, crop, flip, rotate, pad):
        img = img.resize(resize_dims)  # 704, 396
        img = img.crop(crop)  # (left, top, right, bottom) 0, 70, 704, 326
        if flip:
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: Normalized results, 'img_norm_cfg' key is added into
                result dict.
        """
        results['img'] = [mmcv.imnormalize(  # transform -> mean, scale; now is BGR
            img, self.mean, self.std, self.to_rgb) for img in results['img']]
        # results['img'] = [self.imnormalize_(  # transform -> mean, scale
        #     img, self.mean, self.std, self.to_rgb) for img in results['img']]  # bgr data 
