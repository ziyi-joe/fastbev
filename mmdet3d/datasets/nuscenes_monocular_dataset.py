        ...
        for i, result in enumerate(results):
            ...
            try:
                try:
                    ...
                    bev_box_pred = result['boxes_3d'].numpy()[:, [0, 2, 6, 4]][..., :2][scores > thr]
            ...
            if type(results[0]['boxes_3d']) == torch.Tensor:
                return
            ###### draw 3d box in image ######
...
def sort_list(_list, sort):
    ...
    if len(_list) != len(sort):
        import warnings
        warnings.warn("Length of _list is not equal to length of sort", UserWarning)
    new_list = []
    for s in sort:
