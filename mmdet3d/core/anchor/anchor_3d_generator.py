...
class Anchor3DRangeGenerator(object):
    ...
    def grid_anchors(self, featmap_sizes, device='cuda'):
        ...
        for i in range(self.num_levels):
            ...
            if self.reshape_out:
                anchors = anchors.reshape(-1, anchors.size(-1))  # 1, 100, 100, 4, 2, 9 -> 80000,9
        ...
        for anchor_range, anchor_size in zip(self.ranges, self.sizes):
            mr_anchors.append(
                self.anchors_single_range(
                    ...
                    self.rotations,
                    device=device))
        mr_anchors = torch.cat(mr_anchors, dim=-3)  # 1, 100, 100, 1, 2, 9 * 4 -> 1, 100, 100, 4, 2, 9
        return mr_anchors
...
class AlignedAnchor3DRangeGenerator(Anchor3DRangeGenerator):
    ...
    def anchors_single_range(self,
        ...
        rets = torch.meshgrid(x_centers[:feature_size[2]],
                              ...
                              z_centers[:feature_size[0]], rotations)  # 100,100,1,2 * 4
        ...
        tile_shape[-2] = int(sizes.shape[0])
        for i in range(len(rets)):
            rets[i] = rets[i].unsqueeze(-2).repeat(tile_shape).unsqueeze(-1)  # 100,100,1,2 * 4 -> 100,100,1,1,2,1 *4

        sizes = sizes.reshape([1, 1, 1, -1, 1, 3])  # 1,3 -> 1, 1, 1, 1, 1, 3
        tile_size_shape = list(rets[0].shape)
        tile_size_shape[3] = 1
        sizes = sizes.repeat(tile_size_shape)  # 1, 1, 1, 1, 1, 3 -> 100, 100, 1, 1, 2, 3
        rets.insert(3, sizes)

        ret = torch.cat(rets, dim=-1).permute([2, 1, 0, 3, 4, 5])  # 3 + 4 = 7 ; 100, 100, 1, 1, 2, 7 -> 1,100,100,1,2,7
        ...
        if len(self.custom_values) > 0:
            ...
            # custom[:] = self.custom_values
            ret = torch.cat([ret, custom], dim=-1)  # 1,100,100,1,2,7 -> 1,100,100,1,2,9
        return ret
