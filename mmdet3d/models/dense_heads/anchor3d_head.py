...
from mmdet3d.core import (PseudoSampler, box3d_multiclass_nms, limit_period,
                          xywhr2xyxyr, box3d_multiclass_scale_nms)
...
class Anchor3DHead(BaseModule, AnchorTrainMixin):
    ...
    def __init__(self,
        ...
        for i in range(3):
            print('det head transpose: {}'.format(is_transpose))
        self._init_layers()
    ...
    def get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, dir_cls_pred, anchors in zip(
            ...
            cls_score = cls_score.permute(1, 2,
                                          0).reshape(-1, self.num_classes)  # 80,100,100, -> 100,100,80, -> 80000,10 -> sigmoid
            if self.use_sigmoid_cls:
                scores = cls_score.sigmoid()
            else:
                scores = cls_score.softmax(-1)
            bbox_pred = bbox_pred.permute(1, 2,
                                          0).reshape(-1, self.box_code_size)  # 72,100,100, -> 100,100,72 -> 80000,9
            ...
            if nms_pre > 0 and scores.shape[0] > nms_pre:
                if self.use_sigmoid_cls:
                    ...
                    max_scores, _ = scores[:, :-1].max(dim=1)  # 80000,10 -> 80000
