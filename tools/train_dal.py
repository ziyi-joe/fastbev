...
import bst_dal  # NOTE: DAL (STEP 1)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='automatically scale lr with the number of gpus')
    parser.add_argument(
        '--accelerator-config', type=str,
        help='Path to accelerator config file'  # NOTE: DAL (STEP 2)
    )
...
def main():
    ...
    accelerator = bst_dal.get_accelerator(
        config_path=args.accelerator_config  # NOTE: DAL (STEP 3)
    )

    # Optimize model
    accelerator.set_model(model)  # NOTE: DAL (STEP 4)
