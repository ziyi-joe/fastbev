...
def get_img_data_from_origin_path(img_path):
    ...
    for filename in sorted(os.listdir(img_path)):
        ...
        if os.path.isfile(img_file):
            # imgx = cv2.imread(img_file)
            # img_datax.append(cv2.cvtColor(imgx, cv2.COLOR_BGR2RGB))  # cv2 -> BGR to RGB; PIL -> RGB to BGR

            img = Image.open(img_file)  
            # img_0 = np.asarray(img).transpose(2, 0, 1).flatten()
            img_data.append(cv2.cvtColor(np.asarray(img), cv2.COLOR_BGR2RGB)) 
...
def normalize_img(img, mean, std, to_rgb=True):
    ...
    # img[0] = (img[0] - mean[0][0]) * stdinv[0][0]
    # img[1] = (img[1] - mean[0][1]) * stdinv[0][1]
    # img[2] = (img[2] - mean[0][2]) * stdinv[0][2]
...
def main():
    ...
    ############################################ backbone resnet + fpn ############################################
    onnx_model_path_2d = '../tmp/quant_dir/2d_quant.onnx'  # TODO
    providers = ['CUDAExecutionProvider']
    sess = onnxruntime.InferenceSession(onnx_model_path_2d, providers=providers)
    ...
    # compare A1000B0 2d output
    a0 = np.fromfile("output/A1000B0_output/A1000B0_2d_output/model2D_output_0_0_ts_0.bin", dtype=np.int8)
    a0 = a0.reshape(1, 64, 176, 64).transpose(0, 3, 1, 2)*0.0625
    np.array_equal(all_outputs[0], a0)

    # for i in range(24):
    #     base_path = '/workspace/clean_up/fastbev/debug/2d_output_0402/'
    #     first_part = i // 6
    #     second_part = i % 6
    #     file_name = f"{first_part}_{second_part}.npy"
    #     np.save(base_path + file_name, all_outputs[i].cpu())
    ...
    onnx_model_path_3d = '../tmp/quant_dir/3d_quant.onnx'  # TODO
    # model = bstnnx.load(onnx_model_path)
    ...
    # compare A1000B0 3d input
    # b0 = np.fromfile("/workspace/clean_up/release/fastbev/output/3d_input/bev_3d_input_0_0.bin", dtype=np.int8)
    # b0 = b0.reshape(1, 200, 200, 256).transpose(0, 3, 1, 2)*0.0625
    # np.array_equal(input_0, b0)

    providers = ['CUDAExecutionProvider']
    sess = onnxruntime.InferenceSession(onnx_model_path_3d, providers=providers)
    # input_name = sess.get_inputs()[0].name
    ...
    # compare A1000B0 3d output
    dequant_scale = [0.25, 0.03125, 0.125]
    a1000_last = np.fromfile("/workspace/clean_up/release/fastbev/output/A1000B0_output/stage3_output_0.bin", dtype=np.int8)
    a1000b0_out1 = (a1000_last[:80*100*100]*dequant_scale[0]).reshape(1, 100, 100, 80).transpose(0, 3, 1, 2)  # NHWC -> NCHW
    a1000b0_out2 = (a1000_last[80*100*100:(80*100*100*2)]*dequant_scale[1]).reshape(1, 100, 100, 80)[:, :, :, :72].transpose(0, 3, 1, 2)
    a1000b0_out3 = (a1000_last[80*100*100*2:]*dequant_scale[2]).reshape(1, 100, 100, 16).transpose(0, 3, 1, 2)
    np.array_equal(a1000b0_out1, out1)
    np.array_equal(a1000b0_out2, out2)
    np.array_equal(a1000b0_out3, out3)
    ############################################ 3d neck ############################################
